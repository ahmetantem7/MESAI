...
export default function KioskClient() {
  ...
  const deviceId = params.get('device_id') || 'DVN-????'
  ...
  <Image
    ...
    priority
  />
  <h1 style={{ fontSize: 28, fontWeight: 700 }}>MESAI Kiosk</h1>
</div>
        <div style={{ marginTop: 6, opacity: 0.8 }}>Device ID: <b>{deviceId}</b></div>
      </div>
      <div style={{ textAlign: 'right' }}>
        <div style={{ fontSize: 14, opacity: 0.7 }}>Durum</div>
        <div style={{ fontSize: 18, fontWeight: 700 }}>
          {state.phase === 'idle' && 'RFID Bekleniyor'}
          {state.phase === 'authenticated' && 'Operatör Giriş Yaptı'}
          {state.phase === 'running' && 'Çalışıyor'}
          {state.phase === 'paused' && (state.reason === 'break' ? 'Molada' : 'Arızada')}
        </div>
      </div>
    </div>
  )
